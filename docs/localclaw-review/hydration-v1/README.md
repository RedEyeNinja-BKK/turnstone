...
The goal is a ~3× reduction in static per-session prefix tokens (measured ≈73K → target ≈21–24K) with **no regression** in architecture correctness, authority boundaries, evidence semantics, factual continuity, retrieval correctness, or fail-closed behavior.
...
| Default skill `Turnstone Core Identity` | 7,963 | ~2.0K | make named/on-demand |
...
- **Stable baseline = pointer manifest**, never a copied facts database.
...
- Default skill disposition: **Option 2 — `Turnstone Core Identity` becomes activation=`named` / on-demand**.
...
| `core-identity-skill.proposed.md` | Exact proposed post-Hydration Core Identity skill content + disposition |
...
| `quality-gate.md` | Seven pre/post correctness cases with invariants and regression definitions |

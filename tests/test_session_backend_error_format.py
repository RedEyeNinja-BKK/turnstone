...
def test_connect_error_on_removed_alias_classifies_registry_removal(exc_cls):
    """A. alias disabled/removed + connect-class exception -> registry-removal
    classification, NOT provider-unreachable (registry incident 2026-08-15)."""
    stub = _stub()
...
def test_connect_error_on_valid_alias_stays_provider_unreachable(exc_cls):
    """B. alias still valid + genuine upstream connection failure ->
    provider-unreachable classification (registry guard must not mask it)."""
    stub = _stub()
...
def test_removed_alias_timeout_is_not_misclassified(exc_cls):
    """C. guard is connect-branch-only: a timeout on a removed alias must not
    be rewritten to a registry-removal wording."""
    stub = _stub()
    stub._registry = _registry_stub(present=False)
    stub._kind = None
...
def test_connect_error_on_fallback_alias_stays_provider_unreachable(exc_cls):
    """D. fallback behavior unchanged: a connect error for a non-primary
    (fallback) alias is never rewritten to the session's own registry
    removal, even when the session's primary alias is absent."""
    stub = _stub()
    stub._registry = _registry_stub(present=False)
    stub._kind = None
    lane = stub._lane
